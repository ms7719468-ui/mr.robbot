...
def self_ping():
    url = os.getenv('REPL_URL') or (
        f"https://{os.getenv('REPL_SLUG')}.{os.getenv('REPL_OWNER')}.repl.co"
        if os.getenv('REPL_SLUG') and os.getenv('REPL_OWNER') else None)
    if not url:
        print(
            '⚠️ Set REPL_URL in Secrets or ensure REPL_SLUG/REPL_OWNER are available'
        )
        return
    while True:
        try:
